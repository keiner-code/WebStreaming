<template>
  <div class="flex md:flex-row flex-col mt-10">
    <div class="w-full md:w-1/2">
      <h1 class="text-center text-xl mt-2 font-medium">Soportes disponibles</h1>
      <ol class="ml-4 text-lg mt-4">
        <li class="list-decimal my-2">Soporte Técnico Nivel 1 (T1)</li>
        <li class="list-decimal my-2">Especialista en Seguridad Informática</li>
        <li class="list-decimal my-2">Especialista en Aplicaciones</li>
        <li class="list-decimal my-2">Administrador de Sistemas</li>
        <li class="list-decimal my-2">Especialista en Aplicaciones</li>
        <li class="list-decimal my-2">Técnico de Telecomunicaciones:</li>
        <li class="list-decimal my-2">Especialista en Aplicaciones</li>
      </ol>
      <div class="flex justify-center mt-8 mb-4 md:mb-0">
        <button
          class="shadow-xl border border-blue-500 text-blue-500 text-lg px-2 py-1 rounded-lg dark:bg-white font-medium dark:border-none dark:text-black"
        >
          Mas informacion
        </button>
      </div>
    </div>
    <div class="w-full md:w-1/2">
      <img
        class="rounded-xl"
        src="https://img.freepik.com/foto-gratis/retrato-mujer-trabajadora-servicio-al-cliente_144627-37943.jpg?w=740&t=st=1696808514~exp=1696809114~hmac=c0a51a3228da77a59ef8d79ea64aebec7776eb97719e11befdee2c91de3593d5"
        alt="support"
      />
    </div>
  </div>
</template>

<script setup lang="ts"></script>
